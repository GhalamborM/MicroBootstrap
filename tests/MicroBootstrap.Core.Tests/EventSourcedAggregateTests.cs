...
public class EventSourcedAggregateTests
{
    ...
    public void apply_event_should_update_state_and_add_event_to_uncommitted_events_with_increase_current_version()
    {
        ...
        shoppingCard.GetUncommittedEvents().Count.Should().Be(2);
        shoppingCard.DomainEvents.Count.Should().Be(2);
        shoppingCard.DomainEvents.Last().Should().BeOfType<ProductItemAddedToShoppingCart>();
        ((Guid)shoppingCard.DomainEvents.Last().AggregateId).Should().Be(shoppingCard.Id);
        shoppingCard.DomainEvents.Last().AggregateSequenceNumber.Should().Be(1);
    }
    ...
    public void flush_uncommitted_should_return_all_uncommitted_event_and_clear_this_list()
    {
        ...
        events.Last().Should().BeOfType<ProductItemAddedToShoppingCart>();
